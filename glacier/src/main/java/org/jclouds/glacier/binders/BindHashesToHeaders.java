...
import org.jclouds.glacier.util.TreeHasher;
...
public class BindHashesToHeaders implements Binder {
   ...
   private HttpRequest addChecksumHeaders(HttpRequest request, Payload payload) {
      try {
         TreeHasher hasher = new TreeHasher(payload);
         request = request.toBuilder()
               .addHeader(GlacierHeaders.LINEAR_HASH, hasher.getHash())
               .addHeader(GlacierHeaders.TREE_HASH, hasher.getTreeHash())
               .build();
      } catch (IOException e) {
